...
import { BadRequestError, NotFoundError, OrderStatus, requireAuth , validateRequest } from '@ttiicckkeett/common';
...
async (req: Request, res: Response) => {
  ...
  // Calculate the expiration data of this order
  ...
  new OrderCreatedPublisher(natsWrapper.client).publish({
    ...
    status: order.status,
    userId: order.userId,
    expirseAt: order.expiresAt.toISOString(),
    ticket: {
      id: ticket.id,
      price: ticket.price
    }
